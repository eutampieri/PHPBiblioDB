...
def gBooksParser(isbnapi, inforeq):
	...
	try:
		...
		gbooksjson=json.loads(jsonurl.read())
          ...
          if inforeq=="copertina":
            neterror=open('internet.jpg')
            gapi= Response(response=neterror.read(), status=200,mimetype="image/jpeg")
          else:
            gapi=Response(response="Nessuna connessione a Internet", status=200, mimetype="text/plain")
	else:
		try:
			a=gbooksjson["items"]
			book=a[0]
			...
			if str(inforeq)=="titolo":
				...
				try:
                  ...
                  if inforeq =="copertina":
                    ...
                    gapi=Response(response="", status=200, mimetype="text/plain")
	return gapi
